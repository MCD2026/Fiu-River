...
$pythonScript = Join-Path $PSScriptRoot 'rebuild_fiu_site.py'
...
try {
    ...
    if ([IO.Path]::GetExtension($resolvedSource).Equals('.zip', [StringComparison]::OrdinalIgnoreCase)) {
        $temporaryRoot = Join-Path ([IO.Path]::GetTempPath()) ("fiu-river-rebuild-" + [Guid]::NewGuid().ToString('N'))
    ...
    Invoke-Checked { & $pythonCommand.Source @arguments } 'The Fiu River rebuild failed.'
    ...
    if ($Publish) {
        ...
        Push-Location $resolvedRepo
        try {
            git add -- github-pages/index.html github-pages/3d/index.html github-pages/data/Fiu_Photos.js github-pages/map github-pages/terrain
            git diff --cached --quiet
            if ($LASTEXITCODE -eq 0) {
                Write-Host 'No website changes were detected; nothing needs publishing.'
                exit 0
            }
            $message = 'Update Fiu River stakeholder map ' + (Get-Date -Format 'yyyy-MM-dd')
            ...
            Write-Host 'Stakeholder link: https://mcd2026.github.io/Fiu-River/'
