...
QGIS_PREFIX = "var json_output_1 = "
SITE_PREFIX = "var json_Fiu_Photos = "
...
def parse_js_collection(path: Path, prefix: str) -> dict:
...
def qgis_image_lookup(images_dir: Path) -> dict[str, Path]:
    ...
    for path in sorted(images_dir.iterdir()):
        ...
        if current is None or (path.suffix.lower() == ".jpeg" and current.suffix.lower() != ".jpeg"):
            lookup[key] = path
    return lookup
...
def find_qgis_export(source: Path) -> Path | None:
    candidates = []
    for data_file in source.rglob("output_1.js"):
        if data_file.parent.name.lower() != "layers":
            continue
...
def records_from_qgis(root: Path) -> list[PhotoRecord]:
    collection = parse_js_collection(root / "layers" / "output_1.js", QGIS_PREFIX)
    images = qgis_image_lookup(root / "images")
    ...
    for feature in collection.get("features", []):
        ...
        if not filename:
            continue
        existing = unique.get(filename.lower())
        if existing is None or (photo.lower().endswith(".jpeg") and not str(existing["properties"].get("photo", "")).lower().endswith(".jpeg")):
            unique[filename.lower()] = feature

    records = []
    for feature in unique.values():
        properties = feature["properties"]
        filename = str(properties["filename"])
        image = images.get(filename.lower())
        if image is None:
...
def record_from_original(path: Path) -> PhotoRecord:
...
def records_from_originals(source: Path) -> list[PhotoRecord]:
    ...
    for path in image_paths:
        try:
            records.append(record_from_original(path))
...
def load_existing(site: Path) -> list[dict]:
    data_path = site / "data" / "Fiu_Photos.js"
...
def write_dataset(site: Path, features: list[dict]) -> None:
    collection = {"type": "FeatureCollection", "name": "Fiu River Photos", "features": features}
    text = SITE_PREFIX + json.dumps(collection, separators=(",", ":")) + ";\n"
    for path in (site / "data" / "Fiu_Photos.js", site / "map" / "data" / "Fiu_Photos.js"):
...
def download_terrain(task: tuple[int, int, int, Path]) -> int:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "MCD-Fiu-River-rebuild/1.0"},
    )
...
def cache_terrain(features: list[dict], destination: Path, max_zoom: int = 15, buffer: int = 1) -> None:
    ...
    for zoom in range(max_zoom + 1):
        north_west = tile_xy(west, north, zoom)
        south_east = tile_xy(east, south, zoom)
        limit = 2**zoom - 1
        min_x = max(0, min(north_west[0], south_east[0]) - buffer)
        max_x = min(limit, max(north_west[0], south_east[0]) + buffer)
        min_y = max(0, min(north_west[1], south_east[1]) - buffer)
        max_y = min(limit, max(north_west[1], south_east[1]) + buffer)
        ranges[str(zoom)] = {"minX": min_x, "maxX": max_x, "minY": min_y, "maxY": max_y}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Rebuild or append to the Fiu River stakeholder map.")
    ...
    existing_names = {str(feature.get("properties", {}).get("filename", "")).lower() for feature in existing}
    additions = [record for record in incoming if record.filename.lower() not in existing_names]
